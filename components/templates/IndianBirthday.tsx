...
const C = {
  ...
  textFaint: 'rgba(255,248,240,0.28)',
  saffron: '#FF8C00',
  saffronMuted: 'rgba(255,140,0,0.6)',
  saffronFaint: 'rgba(255,140,0,0.1)',
  saffronBorder: 'rgba(255,140,0,0.25)',
  purple: '#9C27B0',
  purpleMuted: 'rgba(156,39,176,0.55)',
  purpleFaint: 'rgba(156,39,176,0.1)',
  purpleBorder: 'rgba(156,39,176,0.25)',
  gold: '#FFD700',
  goldMuted: 'rgba(255,215,0,0.6)',
  goldFaint: 'rgba(255,215,0,0.1)',
...
const STARS = Array.from({ length: 60 }, (_, i) => ({
  ...
  r: Math.random() * 1.6 + 0.4, delay: Math.random() * 5, dur: 2 + Math.random() * 3.5,
  color: i % 3 === 0 ? C.gold : i % 3 === 1 ? C.saffron : '#FF6FA3',
}))
    ...
    <div className="pointer-events-none absolute inset-0 overflow-hidden" aria-hidden>
      {STARS.map((s) => (
        <motion.div key={s.id} className="absolute rounded-full"
          ...
          animate={{ opacity: [0.06, 0.8, 0.06], scale: [1, 1.4, 1] }}
...
// ── Confetti sparkles rising ──────────────────────────────────
const SPARKS = Array.from({ length: 14 }, (_, i) => ({
  id: i, x: 10 + Math.random() * 80, delay: Math.random() * 8, dur: 8 + Math.random() * 6,
  drift: (Math.random() - 0.5) * 60, color: [C.saffron, C.gold, '#FF6FA3', '#00E5FF', '#76FF03'][i % 5],
}))

const RisingSparkles = memo(function RisingSparkles() {
  const reduced = useReducedMotion()
  if (reduced) return null
  return (
    <div className="pointer-events-none absolute inset-0 overflow-hidden" aria-hidden>
      {SPARKS.map((s) => (
        <motion.div key={s.id} className="absolute select-none text-xs" style={{ left: `${s.x}%`, bottom: -20, color: s.color }}
          animate={{ y: -800, x: [0, s.drift, s.drift * 0.5, -s.drift * 0.3, 0], opacity: [0, 0.9, 0.6, 0.2, 0], rotate: [0, 180, 360] }}
...
const FestiveDivider = memo(function FestiveDivider({ className = '' }: { className?: string }) {
  return (
    <div className={`flex items-center justify-center gap-2 ${className}`}>
      <div className="h-px w-12" style={{ background: `linear-gradient(90deg,transparent,${C.saffronMuted})` }} />
      <span className="select-none text-[9px]" style={{ color: C.purple }}>★</span>
      <div className="h-px w-6" style={{ background: `linear-gradient(90deg,${C.saffronMuted},${C.goldMuted})` }} />
      <span className="select-none text-[11px]" style={{ color: C.gold }}>✦</span>
      <div className="h-px w-6" style={{ background: `linear-gradient(270deg,${C.saffronMuted},${C.goldMuted})` }} />
      <span className="select-none text-[9px]" style={{ color: C.purple }}>★</span>
      <div className="h-px w-12" style={{ background: `linear-gradient(270deg,transparent,${C.saffronMuted})` }} />
    </div>
  )
})
...
const MusicButton = memo(function MusicButton({ src }: { src: string }) {
  ...
  return (
    <motion.button onClick={toggle} whileHover={{ scale: 1.04 }} whileTap={{ scale: 0.96 }}
      ...
      style={{ background: C.bgSurface, border: `1px solid ${C.saffronBorder}`, color: C.text }}>
      ...
      <div className="flex items-end gap-[3px] h-4">
        {[4, 7, 10, 7, 4].map((h, i) => (
          <motion.div key={i} className="w-[3px] rounded-full" style={{ background: playing ? C.saffron : C.textFaint }}
...
function BirthdayWishes({ eventId }: { eventId: string }) {
    ...
    <section className="px-4 sm:px-6 md:px-8 py-12 sm:py-18 md:py-24" style={{ background: C.bgMid }}>
      <div className="max-w-xl mx-auto">
        <motion.div {...fadeUp()} className="text-center mb-14">
          ...
          <h2 className="font-heading text-3xl sm:text-4xl mb-6" style={{ color: C.text }}>Birthday Wishes</h2>
          <FestiveDivider />
        </motion.div>
          ...
          <AnimatePresence mode="wait">
            {submitted ? (
              <motion.div key="thanks" initial={{ opacity: 0, scale: 0.96 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0 }} className="text-center py-8">
                <motion.div initial={{ scale: 0 }} animate={{ scale: [0, 1.3, 1] }} transition={{ duration: 0.5, ease: BEZIER }} className="text-3xl sm:text-4xl md:text-5xl select-none mb-6" style={{ color: C.saffron }}>★</motion.div>
                <p className="font-heading text-2xl mb-3" style={{ color: C.text }}>Bahut Shukriya!</p>
                <p className="text-sm mb-7" style={{ color: C.textMuted, lineHeight: 1.8 }}>Your wonderful wishes have been received!<br />The birthday star will love them.</p>
              ...
              <motion.form key="form" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onSubmit={handleSubmit} className="space-y-7">
                ...
                <div>
                  ...
                  <textarea value={message} onChange={e => setMessage(e.target.value.slice(0, MAX))} placeholder="Write a birthday wish…" required rows={4} className="w-full border-0 border-b py-3 text-sm focus:outline-none resize-none" style={is} onFocus={fo} onBlur={bl} />
          ...
          <div className="space-y-4">
            {wishes.map((wish, i) => (
              <motion.div key={wish.id} {...fadeUp(i * 0.07)} className="rounded-lg sm:rounded-2xl px-7 py-7 relative overflow-hidden" style={{ background: C.bgCard, border: `1px solid ${C.saffronBorder}` }}>
                ...
                <div className="font-heading select-none mb-2 leading-none" style={{ fontSize: '3.5rem', color: 'rgba(255,140,0,0.1)' }} aria-hidden>&ldquo;</div>
...
export default function IndianBirthday({ data, eventId, isPreview = false }: Props) {
  const celebrant = data.celebrantName || 'Kavya'
  const age = data.age
  const { date, time, venue, venueAddress, mapsUrl, theme, schedule, galleryImages, musicUrl, message } = data
  ...
  const details = [
    ...
    { label: 'Party Time', value: formattedTime },
    ...(venue ? [{ label: 'Venue', value: venue, sub: venueAddress }] : []),
    ...(theme ? [{ label: 'Theme', value: theme }] : []),
  ].filter(d => d.value)
  ...
  return (
    <div className="relative min-h-screen font-body" style={{ background: C.bg, color: C.text }}>
      ...
      <section className={`relative flex ${isPreview ? 'min-h-[360px] py-12' : 'min-h-screen'} flex-col items-center justify-center overflow-hidden px-4 sm:px-6 md:px-8 text-center`}>
        <FilmGrain />
        <StarField />
        {!isPreview && <RisingSparkles />}

        <div className="pointer-events-none absolute inset-0" style={{ background: 'radial-gradient(ellipse 70% 55% at 50% 40%, rgba(255,140,0,0.08) 0%, transparent 65%)' }} aria-hidden />
        <div className="pointer-events-none absolute inset-0" style={{ background: 'radial-gradient(ellipse 50% 40% at 50% 60%, rgba(156,39,176,0.07) 0%, transparent 70%)' }} aria-hidden />
        ...
        {[isPreview ? '260px' : '580px', isPreview ? '180px' : '400px', isPreview ? '110px' : '250px'].map((s, i) => (
          <div key={i} className="pointer-events-none absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 rounded-full"
            style={{ width: s, height: s, border: `1px solid rgba(255,215,0,${0.12 - i * 0.035})` }} aria-hidden />
        ))}
        ...
        <div className="relative z-[2] flex flex-col items-center">
          <motion.p initial={{ opacity: 0, letterSpacing: '1.5em' }} animate={{ opacity: 1, letterSpacing: '0.5em' }} transition={{ duration: 1.4, delay: 0.1, ease: BEZIER }}
            ...
            {age ? `A Grand Celebration of ${age} Years` : 'A Grand Celebration'}
          </motion.p>
          ...
          {musicUrl && !isPreview && (
            <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7, delay: 0.45, ease: BEZIER }} className="mb-9">
          ...
          {age && (
            <motion.div initial={{ opacity: 0, scale: 0.5 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.9, delay: 0.85, ease: BEZIER }}
              className="mt-4 flex items-center justify-center gap-2 sm:gap-3 px-4 sm:px-6 md:px-8 py-2.5 rounded-full"
              style={{ background: C.saffronFaint, border: `1px solid ${C.saffronBorder}` }}>
              <span className="select-none" style={{ color: C.saffron }}>★</span>
              <span className="font-heading text-lg tracking-[0.15em]" style={{ color: C.saffron }}>Turning {age}</span>
              <span className="select-none" style={{ color: C.saffron }}>★</span>
            </motion.div>
          )}
          ...
          <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 1, delay: 1.25 }}
            ...
            {formattedDate || 'The Celebration Day'}
          </motion.p>
          {venue && (
      ...
      {date && (
        <section className={`px-4 sm:px-6 md:px-8 ${isPreview ? 'py-6' : 'py-12 sm:py-16 md:py-20'}`} style={{ background: C.bgMid }}>
          <div className="max-w-lg mx-auto">
            <motion.p {...fadeUp()} className="text-center text-[11px] uppercase tracking-[0.38em] mb-8 sm:mb-10" style={{ color: C.goldMuted }}>Party Begins In</motion.p>
      ...
      {details.length > 0 && (
        <section className={`px-4 sm:px-6 md:px-8 ${isPreview ? 'py-6' : 'py-12 sm:py-16 md:py-20'}`} style={{ background: C.bg }}>
          <div className="max-w-2xl mx-auto">
            <motion.p {...fadeUp()} className="text-center text-[11px] uppercase tracking-[0.38em] mb-3" style={{ color: C.goldMuted }}>Party Details</motion.p>
            <motion.h2 {...fadeUp(0.08)} className={`font-heading text-center ${isPreview ? 'text-lg mb-5' : 'text-2xl sm:text-3xl md:text-4xl mb-8 sm:mb-12'}`} style={{ color: C.text }}>Join the Festivities</motion.h2>
      ...
      {scheduleItems.length > 0 && (
        <section className="px-4 sm:px-6 md:px-8 py-12 sm:py-16 md:py-20" style={{ background: C.bgMid }}>
          <div className="max-w-md mx-auto">
            ...
            <motion.h2 {...fadeUp(0.07)} className="font-heading text-center text-2xl sm:text-3xl mb-8 sm:mb-12" style={{ color: C.text }}>Party Schedule</motion.h2>
      ...
      {gallery.length > 0 && (
        <section className="py-12 sm:py-16 md:py-20 overflow-hidden" style={{ background: C.bg }}>
          ...
          <motion.h2 {...fadeUp(0.07)} className="px-4 sm:px-6 md:px-8 font-heading text-center text-2xl sm:text-3xl mb-8 sm:mb-10" style={{ color: C.text }}>Captured Moments</motion.h2>
      ...
      {message && (
        <section className="px-4 sm:px-6 md:px-8 py-12 sm:py-18 md:py-24" style={{ background: C.bgMid }}>
          <div className="max-w-xl mx-auto text-center">
            <motion.div {...fadeUp()}>
              <div className="font-heading select-none mb-1 leading-none" style={{ fontSize: '6rem', color: 'rgba(255,140,0,0.09)' }} aria-hidden>&ldquo;</div>
              <p className="font-heading text-xl sm:text-2xl italic leading-relaxed" style={{ color: C.textMuted }}>{message}</p>
              <p className="mt-7 text-sm uppercase tracking-[0.32em]" style={{ color: C.goldMuted }}>— {celebrant}</p>
            </motion.div>
          </div>
        </section>
      )}

      {eventId && <BirthdayWishes eventId={eventId} />}
